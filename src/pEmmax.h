...
public:
  ...
  static bool writeKinWithIDs(const char* outname, MatrixXd& K, double wsum, std::vector<std::string>& ids) {
    ...
    int size = N_MAGIC + sizeof(int) + sizeof(int) + sizeof(double) + sizeof(double) * n * ( n + 1 ) / 2;
  ...
  static bool writeEigenWithIDs(const char* outname, MatrixXd& evecs, VectorXd& evals, double trK, std::vector<std::string>& ids) {
    ...
    int r = evecs.rows();
    int c = evecs.cols();
    int size = N_MAGIC + sizeof(int) + sizeof(int) + sizeof(double) + sizeof(double) * ( r * c + c );
    char* p = (char*) malloc(size);
  ...
  static bool readEigenWithIDs(const char* inname, MatrixXd& evecs, VectorXd& evals, double& trK, std::vector<std::string>& ids) {
    ...
    double* p = new double[size];
    int nr = tf.read(p, sizeof(double)*size);
    if ( nr != (int)sizeof(double)*size ) { error("pEmmax::readEigen() - file is truncated"); }
    evecs.resize(r,c);
    evals.resize(c);
    for(k=0; k < c; ++k) {
      evals(k) = p[k];
